...
## Some data prep
For this analysis, I only concentrated on the messenger data which can be found in the folder `messages`. The folder contains some further folders for message requests, stickers etc but the important one is the `inbox` folder containing the messages itself. For each conversation, there is at least one folder. If you ever sent media such as pictures, then there will be two folders, one for the message JSON and one for the media files. The message folder always starts with a lowercase-letter. Unfortunately, Facebook doesn't use [proper JSON encoding](https://stackoverflow.com/questions/50008296/facebook-json-badly-encoded) which makes it quite a hassle to load the data without any decoding errors. For this analysis, I'm actually only using the aggregated messages so I simply ignored any decoding mistakes in the text. I wrote a few functions that load all the data (using a lot of `{{purr}}`) and result in a data frame with the number of messages per chat and per (active) chat participant, per day:


```{r, results='hide'}
library(tidyverse)
data_path <- "data/facebook-corrieaar"
# script with functions can be found ###link
...
```
```{r, echo=F}
ggsave(here::here("static/post/2020/2020-09-04-animated-facebook-messages_files/static-barchart.png"), 
       plot = p %+% ( tween_dcs %>% filter(month == "2020-01-01")), 
       device = png() , width = width, height = height, units = "in", dpi = res)

```

![Static version of the racing bar chart](/post/2020/2020-09-04-animated-facebook-messages_files/static-barchart.png)
...
for (i in 0:nframes) {
  ...
  composition +
  ggsave(here::here(paste0(folder_path, "composition-", str_pad(i,3, pad = "0"), ".png") ),
         width = 12, height = 15, dpi = 50  )

}
```

Afterwards (it takes a while to generate all images), I execute the following command in my bash in the folder with the images:
```{bash, eval=FALSE}
