...
After computing this for every file and merging it the meta data we get a data frame where each row corresponds to one article for which we extracted tracking IDs. We will now use this to compute edges connecting the outlet organizations.
```{r, eval=F, echo=F}
df <- read_csv(here::here("data/disinfo_data/data.csv")) %>%
  mutate(creative_work_id = str_replace(creative_work_id, "/", "") %>%
           str_replace("/", "_")) 
...
If we specify aesthetics, such as color, for either nodes or edges, we then also need to specify this information when using a scale function. To e.g. color the edges we'd need to use `scale_edge_color_*()`  whereas `scale_color_*()` specifies by default the coloring scale for the nodes.
...
Another almost fully connected network consists of organizations for an Arabic audience and are exclusively connected by Google Ads IDs. A smaller close to fully connected network contains various News Front outlets.

Now, I would expected that all components would be close to fully connected or at least similar to the Sputnik network but the second and third largest networks are only loosely connected: The network containing _russian.rt.com_ consists of 20 nodes and the one with _riafan.ru_ of 13 nodes but they both have much lower average degree of around 3 whereas the average degree for the Sputnik Arabic component has an average degree of 15.6
```{r, eval=F}
org_graph %>%
  activate(edges) %>%
...
For the degree computation I only counted one edge between two nodes, that is if two nodes are connected by different ID types, it still only counts as one edge. Normalizing the degree by component size still shows a much higher average degree for the Sputnik network.
