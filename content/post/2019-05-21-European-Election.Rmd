---
title: "Analysing the European Election: Looking at the Candidates"
author: "Corrie"
date: '2019-05-21'
categories:
  - R
...
Unfortunately, European politics are complex and this is also mirrored in the quantity of parties that are up for vote in Germany. In Germany alone, there are 40 parties on the ballot paper and in total 1.380 candidates are trying to get one of the 96 seats allocated for Germany. No joke, my ballot paper was almost as long as I'm tall!
Reading through some of the names on the ballot paper, I thought it might be interesting to do a small data analysis and to compare some of the parties by their candidate composition.
...
Almost all parties have the same candidate list for all of Germany, the only party that has differing lists for each state is _CDU/CSU_. Depending on the analysis, I sometimes collapse _CDU_ and _CSU_ to a single party or only consider their liste for Berlin.
```{r, echo=F}
library(tidyverse)
library(lubridate)
library(scales)
...
While the ballot paper only shows the top ten candidates for each party (or less if the party has less), many of them have more candidates. Interestingly, some parties have ambitiously many:
```{r}
df %>%
...
The _FDP_ (the economic liberal party) and the _CDU_ (conservative party) nominate both almost twice as many candidates as there are seats avaiable! The SPD (social democratic party) only has more candidates than seats since it has many replacement candidates. Die PARTEI is a satire party, so I'm not really surprised they show up with too many candidates. _ÖDP_, a small ecological party that never got elected into the Bundestag, also seems rather ambitious with 96 candidates. Looking them up, I found out the _ÖDP_ does has one seat in the current parliament.

## A gender-balanced parliament?
Right now, about a third of the members of european parliament (MEPs) are women. We can check the data on how good the chances are that the new parliament might sport more women (at least on the German seats). Since we've seen before that some parties have many more candidates than seats available, it might be misleading to look at all candidates. Instead, it gives a better picture to look at the "Spitzenkandidaten", that is the leading candidates
```{r, echo=F}
all <- df %>%
...
_Die Linke_, the left party, has exactly 50% female and male candidates, wheras the Green (_GRÜNE_) party and the _SPD_ have a very slight female bias. All three parties are rather left-leaning. The other three bigger parties, all more conservative, have a male bias that is especially strong for the _FDP_ and _AfD_. The _AfD_ is far-right so I'm not surprised by their poor gender ratio but I didn't expect the _FDP_ to do so poorly as well. I expected them be more on the same line as _CDU/CSU_, around 40% of females.
...
The feminist party _DIE FRAUEN_ has only female candidates but this isn't really surprising given their name. The only other party with a rather extreme female bias is the animal protection party _TIERSCHUTZ hier!_. The list of parties with an extreme male bias is comparable long: 15 parties, so more than a third of all parties on the ballot, have less than 25% female candidates. There are the right-leaning to far-right parties: III. Weg, _AfD_ and _DIE RECHTE_ (interestingly, the neo-Nazi party _NPD_ is not listed here and thus has a better gender ratio than for example the _FDP_). The list further includes many small conservative parties such as _ÖDP_ (ecological party), _BP_ (bavaria party) and _Bündnis C_ (christian party). I found especially curious the family party _FAMILIE_: 0 female candidates, draw your own conclusion what image they have of a family. The parties _SGP_ (socialist equality party) and _BIG_ (party for innovation and equity/fairness) have equality or equity in their name but it doesn't seem to apply to gender equality.
The internet party _PIRATEN_ apparently follows the nerd stereotype with mostly male candidates.


## What are the candidates working?
The data provides the title of each candidate so we can easily check which party has the highest percentage of candidates with a doctorate:
```{r, fig.width=7}
df %>%
  filter(Kennzeichen == "Bewerber" &
           Listenplatz <= 10) %>%
  ...
  arrange(-prop) %>%
  select(party=Gruppenname, percentage=p, prop) %>%
...
df %>%
  ...
  labs(x="", title="Where do most Top Candidates work?") +
  coord_flip()
```

About one third of candidates work in Business Management and Administration which makes the single largest area of employment. One thing to note here is that the job key doesn't provide information if someone works as a CEO or low-level employee, it really only gives the area. The only exception to this is the category Students. By definition, these candidates are right now in education about to earn either a high-school diploma, a bacheor's degree or a PhD. Also, students is the sixth largest group, closley followed by pensioners, both of which are group I didn't expect to be that well reprented in the candidate pool. 

## Generation change
There are a few clichees out there regarding politics and age: Young people don't care about politics (there are many polls showing young people vote at a lesser rate than older people). Politicians are always old people. While this data set is not the right one to answer how much truth there is to these stereotypes, we can check to some degree if young people aspire to become full-time politicians:
```{r}
df <- df %>%
...
df %>%
  ...
  ggplot(aes(x=med, y=Gruppenname)) +
...
The party Volksabstimmung (Democracy by referendum, they used to be far-right) has a median age of 65 years, meaning most of its candidates are older than this. It is even older than the party Graue Panther, a party for pensioners. On the other side is the party DIE DIREKTE that similarly to the party Volksabstimmung supports direct democracy (without any far-right history though) with a median age of 25. The latter party originated from a student initiative and almost all its candidates are students. Surprisingly, the conservative _CSU_ party that probably for many invokes the image of an old, white male politician has a median age that places it in the younger parties, right below Volt.

## Where do you come from?

The data also provides us with the birth place for each candidate which gives us the opportunity to check how many candidates were born in a country other than Germany. Unfortunately, the birth place only names the city but not the country. To obtain the country, I used the Google Geocoding API. Only three cities could not be identified by Googles API which was easy to code by hand.
```{r, eval=F}
library(ggmap)
...
Not very surprisingly, most candidates were born in Gemany:
```{r}
df <- df %>%
...
Now, are some parties more diverse (in terms of birth places of their candidates) than others or are they relatively equally distributed over the parties?
Let's compare the parties by number of distinct birth countries. To make the comparison equal, I restrict to only the top candidates and also restrict the _CDU_ to their top candidates in Berlin. This ensures that each party is represented by at most 10 candidates in this comparison:
```{r}
df %>%
  filter(Listenplatz <= 10 & 
           Kennzeichen == "Bewerber" &
