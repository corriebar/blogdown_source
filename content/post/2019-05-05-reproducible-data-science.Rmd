...
Until I wanted to redo this analysis I had done four months ago and it wasn't working. I had updated my R version in between, now a bunch of packages were missing and even after installing the missing packages, there were still error messages. That was the moment, I wished I had dockerized this analysis. 

## What actually is Docker? 
For the uninitiated, let me try to give a short summary of what Docker is. With Docker, you can wrap everything that's needed for your analysis (or your web app, or your service, or anything basically) in a nice self-contained package. A description of such a package is called image and an instance of such an image is called container. That sounds rather abstract, so let me give an example. In my analysis, I use `RStan` which is a bit more complicated to install compared to a normal R package. Depending on your operating system, you might have to install a C++ compiler before, check that some configuration are properly set up and then compile everything from source. I already went a few time through this process and already messed up a few times as well, so it's not much fun. So instead of just sharing my R script and have people go through the whole installation process when they want to check my analysis, I can set up a Docker image that contains a fixed operating system environment (usually a light-weight Linux version) with the RStan installation already set up. This image can then be shared and it will work everywhere the same. No more situations where your colleague gets an error on her machine but on yours, everything works just fine.

So, what is the difference between image and container? An image is kind of the blueprint. When you create an image, you specify how it is build, what operating system environment to use, what packages to install etc. When you want to run it, you create a container based on this image. I've seen someone compare it to the difference between `install.package()` and `library()`
...
Another use case is when you want to use a more complex library. For example, instead of going through the whole RStan installation process, I can just check online if someone already prepared a docker image for that, download it and just run it from the container, no installing hassle anymore.

So to summarize, Docker is useful to
- share analysises so they run everywhere, out of the box;
- run your code on a server;
- use a library without having to go through the installation hassle.

## Building the image
I'll describe here how to use Docker to build an image of an analysis. As example, I'll use the analysis I mentioned above which uses RStan for the statistical analysis and, as a further complication, some packages for geospatial data that have some complicated dependencies.
...
```
This will iteractively (`-it`) execute the bash inside the container so than we can now run commands in the command line.
...
RUN apt-get update \
   && apt-get install -y --no-install-recommends \
       ...
       libgeos-dev
```
In my case, these were dependencies needed for some plotting functions for geographical data. Since they are needed before the installation of the R plotting packages, the line also goes before the line installing all R packages.

## Including data in your container
So far, we've been sharing a folder from our host with the docker container. This has the advantage that whatever happens in this folders, will happen both in the container as well as on the host machine. If the R script for example generates some plot files and saves them in this folder, they will still be in this folder after you close the docker container. 
Another option would be to specify in the Dockerfile which files to copy inside the container:
```
RUN mkdir /home/rstudio/my-analysis
COPY . /home/rstudio/my-analysis
```
This first creates the folder `my-analysis` and then copies all files from the folder where you build the Dockerfile into this folder.
Unfortunately, if you want to get any files resulting from this analysis, you still need to share the corresponding folder when running the docker container.

## Sharing your container
A simple way to then share your container is to just share the Dockerfile (including all needed file) for example in a Github repo and then any person who wants to run the analysis, runs
```
